...
// div异常检查
int8_t div_flag;
void check_div(word_t op1, word_t op2, bool sign, bool half)
{
  //printf("op1=0x%lx, op2=0x%lx, sign=%d\n", op1, op2, sign);
  if(op2 == 0)
  {
      div_flag = -1;
  }
  ...
  {
    if((op1==0x8000000000000000) && (op2==0xffffffffffffffff) && !half)
    {
        div_flag =  1;
    }
    else if((BITS(op1, 31, 0)==0x80000000) && (BITS(op2, 31, 0)==0xffffffff) && half)
    {
        div_flag =  1;
    }
  }
...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div   , R, check_div(src1, src2, true, false); if(div_flag == 0) R(dest) = S64(src1) / S64(src2); else if(div_flag == 1)  R(dest) = src1; else R(dest) = 0xffffffffffffffff);
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu  , R, check_div(src1, src2, false,false); if(div_flag == 0) R(dest) = U64(src1) / U64(src2); else R(dest) = 0xffffffffffffffff);
  INSTPAT("0000001 ????? ????? 101 ????? 01110 11", divuw , R, check_div(src1, src2, false, true); if(div_flag == 0) R(dest) = SEXT(U32(BITS(src1, 31, 0)) / U32(BITS(src2, 31, 0)), 32); else R(dest) = 0xffffffffffffffff);
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw  , R, check_div(src1, src2, true, true) ; if(div_flag == 0) R(dest) = SEXT(S32(BITS(src1, 31, 0)) / S32(BITS(src2, 31, 0)), 32); else if(div_flag == 1) R(dest) = SEXT(BITS(src1, 31, 0),32); else R(dest) = 0xffffffffffffffff);
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh  , R, R(dest) = (int64_t)(((__int128_t)S64(src1) * (__int128_t)S64(src2)) >> 64) );
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu, R, R(dest) = ((__int128_t)S64(src1) * (__uint128_t)U64(src2)) >> 64 );
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu , R, R(dest) = (uint64_t)(((__uint128_t)U64(src1) * (__uint128_t)U64(src2)) >> 64) );
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem   , R, check_div(src1, src2, true, false); if(div_flag == 0) R(dest) = S64(src1) % S64(src2); else if(div_flag == 1)  R(dest) = 0; else R(dest) = src1);
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu  , R, check_div(src1, src2, false,false); if(div_flag == 0) R(dest) = U64(src1) % U64(src2); else R(dest) = src1);
  INSTPAT("0000001 ????? ????? 111 ????? 01110 11", remuw , R, check_div(src1, src2, false, true); if(div_flag == 0) R(dest) = SEXT(U32(BITS(src1, 31, 0)) % U32(BITS(src2, 31, 0)), 32); else R(dest) = SEXT(BITS(src1, 31, 0), 32));
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw  , R, check_div(src1, src2, true, true) ; if(div_flag == 0) R(dest) = SEXT(S32(BITS(src1, 31, 0)) % S32(BITS(src2, 31, 0)), 32); else if(div_flag == 1) R(dest) = 0; else R(dest) = SEXT(BITS(src1, 31, 0), 32));
